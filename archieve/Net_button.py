...
def sparse_3d_tensor_convert_to_sparse_5d_tensor(repeated_sparse_3D_tensors, num_stations):
    # 用于存储非零元素的索引和对应值
    all_indices = []
    all_values = []
...
def sparse_4d_tensor_convert_to_sparse_5d_tensor(repeated_sparse_4D_tensors, num_stations):
    # 用于存储非零元素的索引和对应值
    all_indices = []
    all_values = []
    ...
    # 遍历所有原点和目的地
    for origin_idx in range(num_stations):
        sparse_tensor = repeated_sparse_4D_tensors[origin_idx][0]
        # 获取当前稀疏张量的非零索引和对应值
        indices = sparse_tensor.indices()
        values = sparse_tensor.values()
        # 更新全局索引
        for i in range(indices.size(1)):
            # 添加索引，转换为5维
            all_indices.append(
                [origin_idx, indices[0, i].item(), indices[1, i].item(), indices[2, i].item(), indices[3, i].item()])
            all_values.append(values[i].item())
    # 转换为张量
    all_indices = torch.tensor(all_indices).T  # 转置为 (5, N)
    all_values = torch.tensor(all_values)
...
class Net_button(torch.nn.Module):

    def __init__(self, cfg, logger):
        super(Net_button, self).__init__()
        self.logger = logger
        self.cfg = cfg
        ...
        self.ENABLE_4D_FEATURES = True
        self.ENABLE_5D_FEATURES = True

        # Initialize the autoencoder
        self.autoencoder = SimpleAutoencoder(input_dim=self.num_nodes*self.num_nodes*3, latent_dim=self.additional_section_feature_dim)
            ...
            for j in range(num_graphs):
                ...
                if cal_choice=="2D":
                    ...
                    if simplified == 1541543154:
                        ...
                        repeated_sparse_5D_tensors_sliced = sparse_3d_tensor_convert_to_sparse_5d_tensor(
                            repeated_sparse_3D_tensors_sliced, num_stations)
                        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                        OD_path_possibility_5D = OD_path_possibility_5D.to(device)
                        repeated_sparse_5D_tensors_sliced = repeated_sparse_5D_tensors_sliced.to(device)
                        ...
                        """
                        flattened_od_section = OD_section_possibility.view(num_origins,
                                                                                 -1)  # 展平为 [num_origins, num_stations * num_stations * num_stations * 3]
                        ...
                        OD_path_possibility_5D = OD_path_possibility_5D.to(device)
                        repeated_sparse_5D_tensors_sliced = repeated_sparse_5D_tensors_sliced.to(device)
                        OD_section_possibility = torch.matmul(repeated_sparse_5D_tensors_sliced, OD_path_possibility_5D)
                        """
                        flattened_od_section = aggregated_OD_section_possibility.view(num_origins, -1)  # 展平为 [num_origins, num_stations * num_stations]
                        """
                        dense_OD_section_possibility = OD_section_possibility.to_dense()
                        flattened_od_section = dense_OD_section_possibility.view(num_origins,-1)  # 展平为 [num_origins, num_stations * num_stations * num_stations * 3]
                elif cal_choice=="4D":
                    if simplified == 1541543154:
                        OD_path_possibility_expanded = OD_path_possibility.unsqueeze(3)
                        OD_path_possibility_5D = OD_path_possibility_expanded.expand(154, 154, 3, 154)
                        repeated_sparse_5D_tensors_sliced = sparse_4d_tensor_convert_to_sparse_5d_tensor(
                            repeated_sparse_4D_tensors_sliced, num_stations)
                        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                        OD_path_possibility_5D = OD_path_possibility_5D.to(device)
                        repeated_sparse_5D_tensors_sliced = repeated_sparse_5D_tensors_sliced.to(device)
                        repeated_sparse_5D_tensors_sliced = repeated_sparse_5D_tensors_sliced.to_dense()
                        ...
                        flattened_od_section = aggregated_OD_section_possibility.view(num_origins, -1)  # 展平为 [num_origins, num_stations * num_stations]
                        """
                        flattened_od_section = OD_section_possibility.view(num_origins,-1)  # 展平为 [num_origins, num_stations * num_stations * num_stations * 3]
                elif cal_choice=="5D":
                    if simplified == 1541543154:
                        OD_path_possibility_expanded = OD_path_possibility.unsqueeze(3)
                        OD_path_possibility_5D = OD_path_possibility_expanded.expand(154, 154, 3, 154)
                        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                        OD_path_possibility_5D = OD_path_possibility_5D.to(device)
                        repeated_sparse_5D_tensors_sliced = repeated_sparse_5D_tensors_sliced[0][0].to(device)
