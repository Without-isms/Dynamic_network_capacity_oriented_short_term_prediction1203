...
from lib import utils_HIAM_new as utils
from lib import metrics
from lib.utils_HIAM_new import collate_wrapper
from archieve.models.Net_new import Net_new
...
def run_model(model, data_iterator, edge_index, edge_attr, device, seq_len, horizon, output_dim):
    ...
    y_od_pred_list = []
    y_do_pred_list = []
    for _, (x_od, y_od, x_do, y_do, unfinished, history, yesterday, xtime, ytime, PINN_od_features, PINN_do_features,
            PINN_od_additional_features, PINN_do_additional_features,OD_feature_array, OD_path_compressed_array,
            Time_DepartFreDic_Array, repeated_sparse_tensors) in enumerate(data_iterator):
        y_od = y_od[..., :output_dim]
        y_do = y_do[..., :output_dim]
        sequences, sequences_y, y_od, y_do = collate_wrapper(x_od=x_od, y_od=y_od, x_do=x_do, y_do=y_do,
                                                             unfinished=unfinished, history=history,
                                                             yesterday=yesterday,
                                                             PINN_od_features=PINN_od_features,
                                                             PINN_do_features=PINN_do_features,
                                                             PINN_od_additional_features=PINN_od_additional_features,
                                                             PINN_do_additional_features=PINN_do_additional_features,
                                                             ...
                                                             repeated_sparse_tensors = repeated_sparse_tensors,
...
class RecurrentGCN(torch.nn.Module):
    def __init__(self, node_features):
        ...
        self.recurrent = DCRNN(node_features, 2, 1)
...
def main(args):
    ...
    model = Net_new(cfg, logger).to(device)
    ...
    for epoch in range(cfg['train']['epochs']):
        ...
        model.train()
        for _, (
        x_od, y_od, x_do, y_do, unfinished, history, yesterday, xtime, ytime, PINN_od_features, PINN_do_features,
        PINN_od_additional_features, PINN_do_additional_features, OD_feature_array, OD_path_compressed_array,
        Time_DepartFreDic_Array, repeated_sparse_tensors) in enumerate(train_iterator):
            optimizer.zero_grad()
            ...
            sequences, sequences_y, y_od, y_do = collate_wrapper(
                ...
                PINN_od_features=PINN_od_features, PINN_do_features=PINN_do_features,
                PINN_od_additional_features=PINN_od_additional_features,
                PINN_do_additional_features=PINN_do_additional_features,
                OD_feature_array=OD_feature_array,
                OD_path_compressed_array =OD_path_compressed_array,
            ...
            for i_sub_features in range(PINN_od_features.shape[0]):
                ...
                RecurrentGCN_trip_prdc = RecurrentGCN(node_features=3)
                RecurrentGCN_trip_prdc.load_state_dict(torch.load("model.pth"))
                ...
                with torch.no_grad():
                    for snap_time, snapshot in enumerate(trip_gnr_signal):
                        ...
                        cost_PINN = cost_PINN + torch.mean((y_hat - y_od_pred_))

                RecurrentGCN_trip_attr = RecurrentGCN(node_features=3)
                RecurrentGCN_trip_attr.load_state_dict(torch.load("model.pth"))
            ...
            loss = loss_od + loss_do + 0.001 * cost_PINN
