...
from archieve.models.Net import Net
try:
...
def run_model(model, data_iterator, edge_index, edge_attr, device, seq_len, horizon, output_dim):
    ...
    y_od_pred_list = []
    y_do_pred_list = []
    for _, (x_od, y_od, x_do, y_do, unfinished, history, yesterday, xtime, ytime,PINN_od_features,PINN_do_features,PINN_od_additional_features,PINN_do_additional_features) in enumerate(data_iterator):
...
def main(args):
    ...
    for epoch in range(cfg['train']['epochs']):
        ...
        model.train()
        for _, (x_od, y_od, x_do, y_do, unfinished, history, yesterday, xtime, ytime, PINN_od_features, PINN_do_features, PINN_od_additional_features, PINN_do_additional_features) in enumerate(train_iterator):
            optimizer.zero_grad()
            ...
            for i_sub_features in range(PINN_od_features.shape[0]):
                ...
                sub_PINN_od_features_list_of_arrays = [sub_PINN_od_features[i] for i in range(sub_PINN_od_features.shape[0])]
                ...
                RecurrentGCN_model = RecurrentGCN(node_features=3)
                RecurrentGCN_model.load_state_dict(torch.load("model.pth"))
                RecurrentGCN_model.eval()
                y_od_pred_gener = y_od_pred.sum(dim=-1)
                with torch.no_grad():
                    for snap_time, snapshot in enumerate(trip_gnr_signal):
                        ...
                        y_od_pred_= y_od_pred_gener[i_sub_features][snap_time]
                        cost_PINN = cost_PINN + torch.mean((y_hat - y_od_pred_) ** 2)
