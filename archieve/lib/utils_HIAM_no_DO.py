...
class DataLoader(object):
    ...
    def __init__(self,
                 ...
                 repeated_sparse_2D_tensors,
                 repeated_sparse_3D_tensors,
                 # repeated_sparse_4D_tensors,
    ...
    def shuffle(self):
        ...
        PINN_od_features = self.PINN_od_features[permutation]
        PINN_od_additional_features, = self.PINN_od_additional_features[permutation]
        ...
        def _wrapper():
            while self.current_ind < self.num_batch:
                ...
                yield (x_od_i, y_od_i, unfinished_i, history_i, yesterday_i, xtime_i, ytime_i,
                       PINN_od_features_i,PINN_od_additional_features_i,
                       OD_feature_array_features_i,
                       # OD_path_compressed_array_i,
                       Time_DepartFreDic_Array_i,
                       repeated_sparse_2D_tensors_i,
                       repeated_sparse_3D_tensors_i,
                       # repeated_sparse_4D_tensors_i,
...
def load_dataset(dataset_dir,
    ...
    for category in ['train', 'val', 'test']:
        ...
        repeated_sparse_2D_tensors = torch.load(os.path.join(dataset_dir, category + '_repeated_sparse_2D_tensors.pt'))
        repeated_sparse_3D_tensors = torch.load(os.path.join(dataset_dir, category + '_repeated_sparse_3D_tensors.pt'))
        # repeated_sparse_4D_tensors = torch.load(os.path.join(dataset_dir, category + '_repeated_sparse_4D_tensors.pt'))
        # repeated_sparse_5D_tensors = torch.load(os.path.join(dataset_dir, category + '_repeated_sparse_5D_tensors.pt'))
    ...
    data['train_loader'] = DataLoader(data['x_train'],
                                      ...
                                      data['repeated_sparse_2D_tensors_train'],
                                      data['repeated_sparse_3D_tensors_train'],
                                      # data['repeated_sparse_4D_tensors_train'],
                                      # data['repeated_sparse_5D_tensors_train'],
                                      batch_size,
                                      shuffle=True)
    data['val_loader'] = DataLoader(data['x_val'],
                                    ...
                                    data['repeated_sparse_2D_tensors_val'],
                                    data['repeated_sparse_3D_tensors_val'],
                                    # data['repeated_sparse_4D_tensors_val'],
                                    # data['repeated_sparse_5D_tensors_val'],
                                    test_batch_size,
                                    shuffle=False)
    ...
    data['test_loader'] = DataLoader(data['x_test'],
                                     ...
                                     data['repeated_sparse_2D_tensors_test'],
                                     data['repeated_sparse_3D_tensors_test'],
                                     # data['repeated_sparse_4D_tensors_test'],
                                     # data['repeated_sparse_5D_tensors_test'],
                                     test_batch_size,
                                     shuffle=False)
    data['scaler'] = scaler
...
def collate_wrapper(x_od, y_od, unfinished, history, yesterday, PINN_od_features,
                    ...
                    # repeated_sparse_4D_tensors,
                    # repeated_sparse_5D_tensors,
                    edge_index, edge_attr, seq_len, horizon, device, return_y=True):
    ...
    for t in range(T - seq_len, T):
        ...
        """cur_repeated_sparse_4D_tensors_coalesced = []
        for sparse_4D_tensors_list in cur_repeated_sparse_4D_tensors:
            cur_repeated_sparse_4D_tensors_coalesced_list = []
            for sparse_4D_tensors in sparse_4D_tensors_list:
                sparse_4D_tensors=sparse_4D_tensors.coalesce()
                cur_repeated_sparse_4D_tensors_coalesced_list.append(sparse_4D_tensors)
            cur_repeated_sparse_4D_tensors_coalesced.append(cur_repeated_sparse_4D_tensors_coalesced_list)"""


        # cur_repeated_sparse_5D_tensors = transposed_repeated_sparse_5D_tensors[t]
        """
        from pprint import pprint
        print("transposed_repeated_sparse_2D_tensors")
        pprint(transposed_repeated_sparse_2D_tensors)
        print("cur_repeated_sparse_2D_tensors")
        pprint(cur_repeated_sparse_2D_tensors)
        """
        batch = Batch.from_data_list([
            Data(x_od=cur_batch_x_od[i],
                 ...
                 repeated_sparse_2D_tensors=cur_repeated_sparse_2D_tensors[i],
                 repeated_sparse_3D_tensors=cur_repeated_sparse_3D_tensors[i],
                 # repeated_sparse_4D_tensors=cur_repeated_sparse_4D_tensors_coalesced[i],
                 # repeated_sparse_5D_tensors=cur_repeated_sparse_5D_tensors[i].coalesce(),
